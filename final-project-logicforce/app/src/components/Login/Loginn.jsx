...
export default function Login() {
  ...
  return (
    <div className={`${styles.container}`}>
      ...
      {/* Error message pops up when invalid credentials are given for user */}
      {invalid_user ? <MessageToast displayMessage={"Invalid Credentials"} /> : null}
  
      {/* Login component */}
      <div className={style.newlogform}>
        <h2> Login </h2>
        <form onSubmit={handleUserLogin} id="register-user-form">
          <div>
            <label className={style.formLabel}>Email ID</label>
            <input
              name="loginEmail"
          ...
          <button className={style.newlog} type="submit">
            Login
          </button>
          <span className="subtle-text"> Do not have an account ? </span>{" "}
          <Link className="link-decor" href="/register">
            Register
          </Link>
        </form>
      </div>
